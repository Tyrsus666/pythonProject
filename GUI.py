...
def check_for_updates():
    ...
    if (response.json()["name"]) != version:
        bo = tk.Tk()
        bo.withdraw()
        messagebox.showinfo("Tool is outdated",
                            "New version is available on github repository. It is highly recommended to update the tool!")
        bo.destroy()
...
def update_tool():
    ...
    updateWin.geometry('200x200')

    # content
    label = ttk.Label(updateWin, text='Update available.')
